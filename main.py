...
logger = configure_logger("Scraping")  # Chargement du logger
...
def main():
    ...
    try:
        ...
        logger.info(f"Il y a {len(category_urls)} catégories de livres sur le site")
    ...
    try:
        ...
        logger.info(f"Il y a {len(books_urls)} livres à extraire")
    except Exception as e:
        logger.error(f"Une erreur s'est produite lors de l'extraction des livres : {e}")

    # Extractions des données de chaque livres stockés dans books_urls
    try:
        data = extract.get_book_info(books_urls)
        logger.info(f"Il y a {len(data)} livres analysés")
    except Exception as e:
        logger.error(f"Une erreur s'est produite lors de l'analyse des livres : {e}")
